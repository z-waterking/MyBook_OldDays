...
## 2026-09-06 · 全库 AI 稿按作者风格重写

- 依据归档原文与最新复评，重新写作全部 38 篇 AI 正文及 38 份修改说明，直接替换唯一当前稿，统一标记 v3；不保留平行旧稿入口。
- 保留具体生活场景、原有关键对白、自嘲与好句，调整人物首次出场、时间线、段落详略和结尾。短段子恢复轻巧形式，避免为篇幅另编故事。
- 历史招聘信息标明时点，亲历与传闻分开；虚构篇章的情节调整、漫画文字改编及较大删减逐篇写入说明，不冒称逐字保留。
- 同步改稿目录说明、目录封面入口、搜索和图片索引，按既有 33 篇阅读顺序重新生成连续书稿。原文档案、38 份规范评价及三个原文榜单保留本轮复评结果。
- 验证：核对 38 篇新旧稿差异、轮次日期、导航与图片，执行全站巡检、运行时路由和书稿可重复生成检查。
- 发布：仅更新本地文件，未提交或发布。

## 2026-09-06 · 全库原文复评与榜单校准

- 逐篇复评 38 篇归档原文，直接更新各篇唯一 review.md，保留六节结构、唯一总分及逐字摘录；不改作者原文和 AI 当前稿。短小品按自身形式评价，不因篇幅设分数上限。
- 同步评分总榜、分布、分类均分；佳句榜重新筛为 28 条，增加骑行与职业视角选句，剔除若干普通总结及高度套用经典的句式。纠正女孩身份、通勤人群与回程视野等语境。
- 复核全部 18 个趣味专题，修正考公名次、未面试不能断言录用、人物缩写误合并、金额和收入单位等；去掉缺乏依据的严重程度、快乐指数及出场次数。摘录密度重算为有文本分母的 37 篇，明确不是文学质量榜，图片正文不计入。
- 足迹补入哈尔滨、雪乡，江西和日本只作未定位补记；修正华山三次旅行及十年同机位、香港住宿通勤、厦门实际登岛、峨眉山时期、济南情节，以及内尔哈未抵达的表述。首页和静态索引同步。
- 佳句数量与插画数量解耦，允许新选句暂不配图；现配 25 幅佳句插画、18 幅趣味插画，保留未使用的旧图片资产。同步插画清单标题与替代文本。
- 巡检新增佳句原文匹配与摘录密度重算，防止后续复评只更新分数而遗漏引用和密度统计。
...
- 页面切换后自动把当前文章维持在侧栏可视区，并展开它所属的合集或散篇分组。
- 运行时路由契约新增侧栏目标断言，覆盖全库原文、评价、AI 改稿和改稿说明。

## 2026-07-13 · 全模块一致性与移动端可读性修复

- 为原文、文章评价、AI 改稿和改稿说明建立统一上下文导航，并修正评价/改稿页的侧栏归属、动态页面标题和分享元信息。
- 完整站内搜索覆盖原文、评价、AI 改稿、改稿说明、成书页和网站栏目。
- `book/reading-order.json` 拆分档案路径与公开阅读路径，成书网页阅读统一指向当前 AI 稿；连续书稿自动下沉文章内部标题并同步规模统计。
- 手机评分榜改为可展开卡片；文章目录新增分类和评分筛选。
- 为正文静态图片生成轻量 WebP 派生图，运行时补齐懒加载、异步解码和上下文替代文本；原始图片继续作为档案保留。
- 巡检新增评价 H1、四类页面增强、完整搜索、成书双路径、图片派生、首页最高分入口和过期说明校验。
...
## 2026-07-13 · 原文详情页增加 AI 改稿入口与评分分布图

- **类型：** AI 改稿入口 / 文章详情页 / 评分可视化 / 响应式交互
- **状态：** 已完成
- **关联提交：** 本条所在提交

### 改动摘要

- 每篇原文详情页标题右侧自动显示“阅读 AI 改稿”，直接跳到一一对应的合集或散篇改稿。
- 入口由 Docsify 根据当前文章路由动态生成，无需给 38 篇原文逐篇写死链接，后续新增文章也可沿用。
- 手机端允许标题与按钮自然换行，保持足够触控高度。
- 清理从 AI 改稿返回原文时可能残留在侧栏条目上的 `AI改稿` 状态标记。
- 将评分总榜的区间表改为水平柱状分布图，直接标注篇数和占比，并保留面向屏幕阅读器的完整分布说明。
- 巡检会对照 38 份规范评价重新计算各区间数量，防止图表随复评失真。

### 验证

- 首页内联脚本、Node.js 与 Python 语法检查通过。
- 网站巡检与连续书稿可重复生成检查通过。
- 本地 Docsify 在桌面和窄屏下检查入口位置、目标路由、返回原文后的侧栏状态与评分柱状图布局。
